    ...
    public SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        http
            ...
            .authorizeHttpRequests(auth -> { // 서버의 리소스에 접근 가능한 권한을 설정함
                ...
                auth.requestMatchers("/inquirys", "/inquirys/searchInquiry","/security/auth/*","/image/**", "/security/auth/login", "/signup/*", "/main/**","/check/**", "/mailSend", "/mailAuthCheck", "/search/**", "/main/test2","/img/**", "/").permitAll();
                auth.requestMatchers("/roomRegister/**").hasAnyAuthority(UserRole.HOST.getRole());
                auth.requestMatchers("/reservation/**", "/inquirys/*").hasAnyAuthority(UserRole.GUEST.getRole());
